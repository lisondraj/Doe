...
export function doeDtcRouteBootstrapScript(): string {
  ...
  return `(function(){try{var path=location.pathname;var extra=path==="/profile"||path==="/onboarding";if(!path.startsWith(${routePath})&&!extra)return;var html=document.documentElement;var body=document.body;html.removeAttribute("data-home-page");html.removeAttribute("data-about-page");html.removeAttribute("data-doeinsure-page");html.removeAttribute("data-doehome-page");html.removeAttribute("data-layout");html.removeAttribute("data-doephone-pinching");html.setAttribute("data-doedtc-page","true");var desktop=window.matchMedia("(min-width: 1024px)").matches||window.innerWidth>=1024;html.setAttribute("data-doedtc-variant",desktop?"desktop":"phone");var brandFooter=extra||path.startsWith(${routePath});if(brandFooter){${profileOverflowBootstrap}}if(desktop){html.removeAttribute("data-doeforvc-always-phone");if(body)body.classList.add("desktop-route");}else{html.setAttribute("data-doeforvc-always-phone","true");if(body)body.classList.remove("desktop-route");if(!brandFooter){${overflowBootstrap}}${viewportBootstrap}}}catch(e){}})();`;
}

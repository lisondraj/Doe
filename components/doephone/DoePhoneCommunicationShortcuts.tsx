...
function ShortcutPill({ shortcut }: { shortcut: DoePhoneCommunicationShortcut }) {
  return (
    ...
    >
      {shortcut.key ? (
        <span
          className={`flex h-[clamp(1.85rem,1.45rem+2.1vmin,2.55rem)] w-[clamp(2rem,1.55rem+2.25vmin,2.75rem)] shrink-0 items-center justify-center rounded-[0.24rem] bg-white text-[clamp(0.72rem,0.58rem+0.72vmin,0.95rem)] font-medium leading-none text-[#1E343A] iphone-page:h-[clamp(2.65rem,2rem+3.2vmin,3.45rem)] iphone-page:w-[clamp(2.9rem,2.2rem+3.45vmin,3.75rem)] iphone-page:rounded-[0.3rem] iphone-page:text-[clamp(1.02rem,0.82rem+1.05vmin,1.32rem)] ${inter.className}`}
        >
          {shortcut.key}
        </span>
      ) : null}
      <span
        className={`whitespace-nowrap text-[clamp(0.62rem,0.5rem+0.62vmin,0.82rem)] font-medium uppercase leading-none tracking-[0.13em] text-white/88 iphone-page:text-[clamp(0.88rem,0.68rem+1.05vmin,1.15rem)] iphone-page:text-white/90 iphone-page:tracking-[0.12em] ${inter.className}`}
      >
        {shortcut.label}
      </span>
    </div>

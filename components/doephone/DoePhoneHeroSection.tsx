...
import {
  DOE_HOME_HERO_SAPPHIRE_PALETTE,
  doeHomeHeroSapphireShaderSurface,
...
export function DoePhoneHeroSection({
  ...
  proto?: boolean;
  /** Desktop — render the iPhone hero background (sapphire shader + agent orb dial). */
  ...
  const isProto = proto;
  /** iPhone-style hero (sapphire shader + orb dial) — mobile always, desktop when opted in. */
  const renderIphoneHero = !isProto && (isMobile || iphoneBackdrop);
  const homeHeroShader = doeHomeHeroSapphireShaderSurface();
      ...
      style={
        {
          ...
          height: heroHeight,
          ...(renderIphoneHero
            ? { backgroundColor: DOE_HOME_HERO_SAPPHIRE_PALETTE.back }
            : isMobile && isProto

...
import { DESKTOP_MAIN_CTA_MENU_ITEMS } from "@/lib/subpage/subpage-nav";
...
export function DesktopMainNavCta({
  ...
  return (
    <div ref={rootRef} className="relative flex shrink-0 items-center">
      <div className="flex items-stretch overflow-hidden rounded-md" style={{ boxShadow: shadow }}>
        <Link
          href={primary.href}
      ...
      {open ? (
        <div
          role="menu"
          className="absolute right-0 top-[calc(100%+0.5rem)] z-[60] min-w-[11.75rem] overflow-hidden rounded-md border border-[#E6E6E6] bg-white py-1 shadow-[0_8px_24px_rgba(0,0,0,0.12)]"
        >
          {DESKTOP_MAIN_CTA_MENU_ITEMS.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              role="menuitem"

...
function ClaimBody({ revealed }: { revealed: boolean }) {
  ...
  const noteParsed = filed ? noteMarkIds.length : Math.max(0, step - 1);
  const noteMeterPct = (noteParsed / Math.max(1, noteMarkIds.length)) * 100;
  ...
  const ticket = (
    <article className="doeinsure-claim__ticket" key={active.id}>
      ...
      {variant === "phone" ? (
        ...
        <div
          className={`doeinsure-claim__note-panel${noteOn ? " is-in" : ""}${noteReading ? " is-reading" : ""}`}
          style={{ "--claim-scan-ms": `${stepMs * noteMarkIds.length}ms` } as CSSProperties}
        >
          <p className="doeinsure-claim__note doeinsure-claim__note--scan">
  ...
  return (
    <>
      ...
      <DoeInsureAppFrame file="Claim desk · Harbor Notes" className="doeinsure-app--claim">
        <div className={`doeinsure-claim${filed ? " is-on" : ""}`}>{ticket}</div>
      </DoeInsureAppFrame>
    </>
  );
}

...
import { PRODUCT2_CLINIC_LABEL, PRODUCT2_NAV_SECTION_LABEL } from "@/lib/product2/product2-nav";
import type { Product2NavItem, Product2NavTab } from "@/lib/product2/product2-nav";

function NavIcon({ id }: { id: Product2NavTab }) {
  const paths: Record<Product2NavTab, React.ReactNode> = {
...
export function Product2SideNav({
  items,
  activeTab,
  onSelect,
}: {
  items: readonly Product2NavItem[];
  activeTab: Product2NavTab;
  onSelect: (tab: Product2NavTab) => void;
}) {
  return (

...
import {
  PRODUCT2_INTEGRATION_FILTERS,
  PRODUCT2_INTEGRATIONS,
  filterProduct2Integrations,
  type Product2IntegrationFilter,
} from "@/lib/product2/product2-integrations";
...
export function Product2AgentsPanel() {
  const [integrationFilter, setIntegrationFilter] = useState<Product2IntegrationFilter>("all");
  const [integrationQuery, setIntegrationQuery] = useState("");

  const filteredIntegrations = useMemo(
    () => filterProduct2Integrations(PRODUCT2_INTEGRATIONS, integrationFilter, integrationQuery),
    [integrationFilter, integrationQuery],
  );

  const activeFilterIndex = PRODUCT2_INTEGRATION_FILTERS.findIndex((item) => item.id === integrationFilter);
        ...
        <aside className="product-agents-integrations" aria-label="Integrations">
          <div className="product-agents-integrations__box">
            ...
            <div className="product-agents-integrations__filter" role="tablist" aria-label="Integration filters">
              ...
              />
              {PRODUCT2_INTEGRATION_FILTERS.map((item) => {

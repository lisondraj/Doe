...
export function DoeHomeNav() {
  const panelId = useId();
  const [open, setOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  const [active, setActive] = useState("#genome");
  const [promoIndex, setPromoIndex] = useState(0);
  const promos = DOEHOME_NAV.promos;
  ...
  return (
    <header className={`doeinsure-nav${scrolled ? " is-scrolled" : ""}${open ? " is-open" : ""}`}>
      ...
      <div className="doeinsure-nav__bar">
        ...
        <div className="doeinsure-nav__end">
          <a className="doeinsure-btn doeinsure-nav__cta" href="#request" onClick={() => go("#request")}>
            <span className="doeinsure-nav__cta-full">{DOEHOME_NAV.cta}</span>
            <span className="doeinsure-nav__cta-short">{DOEHOME_NAV.ctaShort}</span>
          </a>
          <button
            type="button"
            ...
            aria-label={open ? DOEHOME_NAV.menuClose : DOEHOME_NAV.menuOpen}
      ...
      <div className={`doeinsure-nav__panel${open ? " is-open" : ""}`} id={panelId} aria-hidden={!open}>
        <nav aria-label="Doe menu">
          {DOEHOME_NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}

...
function VoiceLiveConvo({
  ...
  const turns =
    convoView === "agent-only"
      ? PRODUCT_LANDING_LIVE_CONVO.filter((turn) => turn.role === "agent")
      : PRODUCT_LANDING_LIVE_CONVO;
...
function VoiceConvoTurn({
  ...
  turn: (typeof PRODUCT_LANDING_LIVE_CONVO)[number];
  showAgentSteps?: boolean;
}) {
...
export function ProductLandingLiveThread({
  className = "",
  showOutcome = true,
  showActions = true,
  showAgentSteps = true,
